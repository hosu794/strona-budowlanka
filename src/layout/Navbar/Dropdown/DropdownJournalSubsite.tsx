...
const DropdownJournalSubsite = () => {
  ...
  const fetchSubsite = React.useCallback(() => {
    axios
      .get(`${API_SERVER}wp-json/api/v1/dropdown/journal/subsite/${params.id}`)
      .then((response) => {
  ...
  React.useEffect(() => {
    fetchSubsite();
    console.log("Journal Dropdown Subsites content: ", subsite);
  }, [fetchSubsite]);
...
export default DropdownJournalSubsite;
